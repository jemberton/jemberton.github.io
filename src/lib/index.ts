...
export const linkify = (element: HTMLElement, router: Router) => {
  ...
  Array.from(links).forEach((link: HTMLAnchorElement) => {
    ...
    if (link.innerText.startsWith('!ref')) {
      ...
      link.parentElement?.classList.add('mx-none', 'my-xs', 'p-none')
      link.classList.add('m-none', 'p-none')
...
export const buildTOC = (element: HTMLElement) => {
  let toc: TOCLink[] = []
  ...
  if (headings.length > 0) {
    Array.from(headings).forEach((heading: HTMLHeadingElement) => {
      ...
      let url = text.toLowerCase().trim().replace(/\s/g, '-').replace(/[^a-zA-Z0-9-]/, '')
